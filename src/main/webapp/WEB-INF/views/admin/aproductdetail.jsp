...
<div class="contents_inner">
 ...
 <!-- 본문 시작 -->
	<div class="main-content">
		<div class="row">
			<div class="col-sm-12">
			<h3 class="productlist text-center"> [상 품 수 정] </h3>
			<form name="fmproduct" id="fmproduct" method="post" action="${pageContext.request.contextPath}/admin/update" enctype="multipart/form-data">
				<input type="hidden" name="book_number" value="${product.book_number}">	
			<table class="table-product">
			<tbody>
				<tr>
					<td> 책 제목 </td> <!-- 필수 -->
					<td><input type="text" value="${product.book_title}" name="book_title" id="book_title" class="form-control"></td>
				</tr>
				<tr>
					<td> 작가 </td> <!-- 필수 -->
					<td><input type="text" value="${product.author}" name="author" id="author" class="form-control"></td>
				</tr>
				<tr>
					<td> 장르 </td>
					<td>
						<select name="genre_code" id="genre_code" class="form-control">
						    <option value="0" ${product.genre_code == '0' ? 'selected' : ''}> 선택 </option>
				...
				<tr>
					...
					<td>
						<select name="grade_code" id="grade_code" class="form-control">
							<option value="0" ${product.grade_code == '0' ? 'selected' : ''}> 선택 </option>
							<option value="1" ${product.grade_code == '1' ? 'selected' : ''}> 최상 </option>
							<option value="2" ${product.grade_code == '2' ? 'selected' : ''}> 상 </option>
							<option value="3" ${product.grade_code == '3' ? 'selected' : ''}> 중 </option>
						</select>
					</td>															
				<tr>
					<td> 정가 </td> <!-- 필수 -->
					<td><input type="number" value="${product.original_price}" name="original_price" id="original_price" class="form-control"></td>
				</tr>
				<tr>
					<td> 판매가 </td>
                	<td><input type="number" value="${product.sale_price}" name="sale_price" id="sale_price" class="form-control" readonly></td>
				</tr>
				<tr>
					<td> 등록자 </td>
					<td>${product.email} </td>
				...
				<tr>
					...
					<td>
						<select name="availability" id="availability" class="form-control">
							<option value="0" ${product.availability == '0' ? 'selected' : ''}> 미판매 </option>
							<option value="1" ${product.availability == '1' ? 'selected' : ''}> 판매 </option>
							<option value="2" ${product.availability == '2' ? 'selected' : ''}> 보류 </option>
						</select>
					</td>					
				</tr>		
				<tr>
					...
					<td>
						<select name="branch_code" id="branch_code" class="form-control">
							<option value="0" ${product.branch_code == '0' ? 'selected' : ''}> 선택 </option>
							<option value="L01" ${product.branch_code == 'L01' ? 'selected' : ''}> 강남점 </option>
							<option value="L02" ${product.branch_code == 'L02' ? 'selected' : ''}> 역삼점 </option>
							<option value="L03" ${product.branch_code == 'L03' ? 'selected' : ''}> 종로점 </option>
						</select>
					</td>	
				</tr>					
				<tr>
					<td> 이미지 </td> <!-- 필수 -->
					<td> <img src="/storage/images/${product.img}" width="100px">
						 <input type="file" name="img" id="img" class="form-control">
					</td>
				</tr>
 				<tr>
					<td colspan="2" align="center">
                    	<input type="button" value="수정" class="btn btn-newproduct" onclick="product_update()">
					</td>
			    </tr>  
			</tbody>
			</table>	
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
	    ...
	    form.addEventListener('submit', function(event) { // 폼 제출 이벤트를 리스닝합니다.
	        ...
	        // 필수 입력 필드의 ID 목록입니다.
	        var requiredFields = ['book_title', 'author', 'original_price'];
  	...
  	{	// <form name="productfrm">을 가리킴 
		document.fmproduct.action = "${pageContext.request.contextPath}/admin/update";
